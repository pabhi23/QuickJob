...
app.post('/api/resume', (req, res) => {
  ...
  connection.query(query, [firstName, lastName, address, jobTitle, linkedinId, JSON.stringify(experience), JSON.stringify(education), JSON.stringify(skills)], (err, result) => {
    ...
    res.status(201).send({ message: 'Resume saved successfully!', resumeId: result.insertId });
  });
});
...
app.post('/api/resumebuilder', (req, res) => {
  ...
  connection.query(query, [firstName, lastName, address, jobTitle, linkedinId, phone, email], (err, result) => {
      if (err) {
          console.error('Error saving user data:', err);
          res.status(500).send('Error saving user data');
      } else {
          res.status(200).send({ message: 'User data saved successfully', userId: result.insertId });
      }
  });
});
...
app.post('/api/experience', (req, res) => {
  ...
  connection.query(query, [userId, company, position, startDate, endDate, isCurrent ? 1 : 0], (err, result) => {
      if (err) {
          console.error('Error saving experience data:', err);
          return res.status(500).send('Error saving experience data');
      }
      res.status(200).send({ message: 'Experience data saved successfully', experienceId: result.insertId });
  });
});
